...
namespace ManejoPresupuesto.Controllers
{
    public class TransaccionesController: Controller
    {
        ...
        private readonly IServiciosReportes servicioReportes;

        public TransaccionesController(IServicioUsuarios servicioUsuarios, 
            IRepositorioCuentas repositorioCuentas, 
            IRepositorioCategorias repositorioCategorias, 
            IRepositorioTransacciones repositorioTransacciones, 
            IMapper mapper, IServiciosReportes servicioReportes)
        {
        ...
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            IEnumerable<ResultadoObtenerPorSemana> transacciosnesPOrSemana = 
                await servicioReportes.ObtenerReporteSemanal(usuarioId, mes, año, ViewBag);

            var agrupado = transacciosnesPOrSemana.GroupBy(x => x.Semana).Select(x => new ResultadoObtenerPorSemana()
            {
                Semana = x.Key,
            ...
            var diasDelMes = Enumerable.Range(1, fechaReferencia.AddMonths(1).AddDays(-1).Day); //creando arreglo de los dias del mes

            var diasSegmentados = diasDelMes.Chunk(7).ToList(); //divide en pequeños arreglos de 7 en 7
